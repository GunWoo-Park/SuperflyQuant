...
def get_coin_pricepairs(url):
    ...
    for name in names:
        resp.append(name)
        # resp[i].replace("USDT","_USDT")
        # resp[i].replace("BTC", "_BTC")
        # resp[i].replace("USDC", "_USDC")
        # resp[i].replace("DAI", "_DAI")
    ...
    for coin in resp:
        coin_symbol.append(coin.replace("_",""))
        coin_price.append(session.best_bid_ask_price(
        symbol=coin_symbol[i]
        )['result'])
        i = i + 1

    coin_list = dict(zip(resp, coin_price))
...
def get_price_for_t_pair(t_pair, prices_json):
    ...
    pair_a_ask = float(prices_json[pair_a]["askPrice"])
    pair_a_bid = float(prices_json[pair_a]["bidPrice"])
    pair_b_ask = float(prices_json[pair_b]["askPrice"])
    pair_b_bid = float(prices_json[pair_b]["bidPrice"])
    pair_c_ask = float(prices_json[pair_c]["askPrice"])
    pair_c_bid = float(prices_json[pair_c]["bidPrice"])
...
def get_depth_from_orderbook(surface_arb):
    ...
    contract1 = contract_1.replace("_","")
    contract2 = contract_2.replace("_","")
    contract3 = contract_3.replace("_","")
    session = HTTP("https://api.bybit.com",
                   spot=True)
    #pprint.pprint(session.orderbook(symbol=sym, limit=20))
    #depth_1_prices = get_coin_tickers(url1)

    depth_1_prices = session.orderbook(symbol=contract1, limit=20)['result']
    ...
    depth_2_prices = session.orderbook(symbol=contract2, limit=20)['result']
    ...
    depth_3_prices = session.orderbook(symbol=contract3, limit=20)['result']
