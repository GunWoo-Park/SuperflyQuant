...
async def my_loop_WebSocket_bybit():
    ...
    async with websockets.connect("wss://stream.bybit.com/spot/quote/ws/v1") as websocket:  # OK websockets version 9.1
        print("Connected to bybit WebSocket");
        #await websocket.send('{"op":"subscribe","args":["trade.BTCUSD"]}');
        await websocket.send('{"op":"subscribe","args":["depth.BTCUSDT"]}');
